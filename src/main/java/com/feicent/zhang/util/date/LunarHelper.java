package com.feicent.zhang.util.date;
...
/**
 * 万年历工具类
 * http://zuoming.iteye.com/blog/1554001
 */
public class LunarHelper {
    ...
    static final String[] CHINESE_TEN = {"初", "十", "廿", "卅"};
    static final String[] LUNAR_GAN = {"甲", "乙", "丙", "丁", "戊", "己", "庚", "辛", "壬", "癸"};
    static final String[] LUNAR_ZHI = {"子", "丑", "寅", "卯", "辰", "巳", "午", "未", "申", "酉", "戌", "亥"};
    ...
    /**
     * @param num
     * @return 传入 月日的offset 传回干支, 0=甲子
     */
    final private static String cyclicalm(int num) {
    	return (LUNAR_GAN[num % 10] + LUNAR_ZHI[num % 12]);
    }
    ...
    public LunarHelper(Calendar cal) {
        ...
        //求出和1900年1月31日相差的天数
        int offset = daysBetween(cal.getTime().getTime()-baseDate.getTime());
        //用offset减去每农历年的天数
        // 计算当天是农历第几天
        //i最终结果是农历的年份
        //offset是当年的第几天
    ...
    public static int daysBetween(long start, long end) {
    	int days = (int) ((end -start)/ONE_DAY_MILLIS);
    	return Math.abs(days);
	}
    
    public static int daysBetween(long millins) {
    	int days = (int) (millins/ONE_DAY_MILLIS);
    	return Math.abs(days);
	}
    
    public static String getChinaDayString(int day) {
    	int index = day % 10 == 0 ? 9 : day % 10 - 1;
        if (day > 30)
            return "";
        if (day == 10)
            return "初十";
        else
            return CHINESE_TEN[day / 10] + CHINESE_NUMBER[index];
    }
    ...
    public static void main(String[] args) {
		...
		LunarHelper lunar = new LunarHelper(cal);
		System.out.println(lunar.getLunarDate());
		System.out.println("农历日期:" + lunar.toString());
		
		Calendar today = Calendar.getInstance();
		LunarHelper lunar2 = new LunarHelper(today);
   	    System.out.println("公历：" + chineseDateFormat.format(today.getTime()) + ",　农历:" + lunar2);
	}
