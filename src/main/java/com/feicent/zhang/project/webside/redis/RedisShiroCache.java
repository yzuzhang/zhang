...
import com.feicent.zhang.project.webside.SerializeUtil;
...
public class RedisShiroCache<K, V> implements Cache<K, V> {
	...
	private static RedisClient redisManager;
    
    private String name;

    
    public RedisShiroCache(String name, RedisClient redisManager) {
        this.name = name;
        RedisShiroCache.redisManager = redisManager;
    }
    ...
    public V get(K key) throws CacheException {
        ...
        try {
            byteValue = RedisClient.getValueByKey(DB_INDEX, generateCacheKey(key).getBytes());
        } catch (Exception e) {
    ...
    public V put(K key, V value) throws CacheException {
        V previos = get(key);
        try {
        	RedisClient.saveValueByKey(DB_INDEX, generateCacheKey(key).getBytes(), SerializeUtil.serialize(value), -1);
        } catch (Exception e) {
    ...
    public V remove(K key) throws CacheException {
        V previos = get(key);
        try {
        	RedisClient.deleteByKey(DB_INDEX, generateCacheKey(key).getBytes());
        } catch (Exception e) {
	...
	public <V> V get(String key , @SuppressWarnings("unchecked") Class<V>...requiredType) throws Exception
    {
    	return RedisClient.get(key, requiredType);
    }
